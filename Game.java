...
public class Game {
    ...
    // stores rooms player has visited as a stack, so only the most recently added
    // room can be accessed, used for 'back' command
    ...
    public Game() {
        ...
        player = new Player();
        player.setGoal(map.getGoals()); // sets goal of player to list of items that need to be collected to win game
                                        // (defined in map)
        ...
        // sets characters of current game to the ones created in map (so they can be
    ...
    private void printWelcome() {
        ...
        System.out.println("Dreamscape is a text-based adventure game.");
    ...
    /**
     * Loads characters by saving their name (used in commands) and their Character
     * object to a HashMap.
     * Used to call character's methods to move and interact with player.
     * 
     * @param List of characters returned from 'getAllCharacters' method in map
     *             (where characters are initailised).
     */
    private void loadCharacters(ArrayList<Character> characterList) {
        characters = new HashMap<>();

        for (Character character : characterList) {
            // saves character in HashMap in format (name used for commands, Character
            // object)
     ...
     * room description, items in room, characters in room, exit information,
     * player's strength bar.
     */
    ...
    private void goBack() {
        if (!roomHistory.empty()) {
            // sets 'previousRoom' to the first room in the room history stack (ie last room
            // to have been visited)
            Room previousRoom = roomHistory.pop();

            // changes player's location to that room
            // without adding room player leaves to stack (ie like retracing steps using
            // string to the initial room)
     ...
     * If item exists player attempts to add it to inventory by checking if it isn't
     * immovable.
     */
    private void takeItem(Command command) {
        ...
        String itemName = command.getWord(2); // supposedly the second word will be the name of the item to be picked up
                                              // (target)
     ...
     * "Drop" was entered.
     * Try to find the target item. If not found an error message is printed.
     * If item exists player attempts to remove it from inventory by checking if it
     * was
    ...
    private void dropItem(Command command) {
        ...
        // dropItem method returns a String with a message saying that item has been
        // dropped to then print
    ...
    private void goRoom(Command command) {
        ...
        if (nextRoom == null) {
            ...
            if (nextRoom instanceof TeleportRoom) {
                ...
                Room roomToTeleportTo = room.teleportTo(map.getAllRooms()); // random room to teleport to from existing
                                                                            // rooms chosen
     ...
     * Change's players current room, adds last room to history, and moves
     * characters.
     */
    ...
    /**
     * Used in goRoom method.
     * Iterates through all characters in game and changes their current location
     * changed to a random neighbouring room.
     */
    private void moveCharacters() {
        // all characters in the game move to another random room (that was neighbouring
        // their current room)
     ...
     * If player has all the goal items and is in correct end room, player wins (won
     * boolean set to true), ending the game
     */
    private void wakeUp(Command command) {
        ...
        if ((command.getWord(2)).equals("up")) {
            ...
            // if player doesn't have all the requirements message saying they haven't won
            // yet printed
            if (!hasWon) {
    ...
    /**
     * "talk" was entered.
     * Tries to talk to the character specified, if this exists, if not an error
     * message printed.
     */
    private void talkTo(Command command) {
        ...
        } else if (character.isInRoom(player.getCurrentRoom())) { // checks if the character is in the same room as the
                                                                  // player
            System.out.println(character.getTalk()); // prints the dialogue of character
            System.out.println(character.giveIfHappy(player)); // if the character is happy they may give a reward to
                                                               // the player
        } else {
    ...
    /**
     * "give" was entered.
     * The command word order is expected to be: "give [item in inventory]
     * [character to give it to]"
    ...
    private void giveItem(Command command) {
        ...
        // if character specified is in the room AND player has specified item in
        // inventory, action can be completed
        if (character.isInRoom(player.getCurrentRoom()) && player.getInventory().hasItemString(itemName)) {
            // character recieves item specified (checking if it is edible - if so character
            // is now happy) String with reply returned
            ...
            // description of the action - response of character and removal of item -
            // printed
    ...
    /**
     * "use" was entered.
     * The command word order is expected to be: "use [item in inventory]" OR "use
     * [item in inventory] [item in room to use with]"
     * Depending on the number of words in the command, try to use the item(s) and
     * print
     * result returned.
     */
     ...
     * 
     * NOTE: moveTo(player.getCurrentRoom()) method could have been used BUT this
     * would cause player's
